...
void handleConfigWifi()               //返回http状态
{
  ...
  if (server.hasArg("ssid"))          //判断是否有账号参数
  {
    ...
    if ((0 == wifi_ssid.length()) || (wifi_ssid.length() > SSID_LENGTH)) {
      serial_writelog("invalid ssid length:%u\r\n", wifi_ssid.length());
      server.send(400, "text/html", "<meta charset='UTF-8'>WiFi 名称不能为空且不能超过 32 字节");
      return;
    }
    serial_writelog("%s\r\n",wifi_ssid.c_str());
  } 
  ...
  if (server.hasArg("password")) 
  {
    ...
    if (wifi_pass.length() > WIFI_PASSWORD_LENGTH) {
      serial_writelog("invalid password length:%u\r\n", wifi_pass.length());
      server.send(400, "text/html", "<meta charset='UTF-8'>WiFi 密码不能超过 64 字节");
      return;
    }
    serial_writelog("got password\r\n");
  } 
  else 
  {
  ...
  server.send(200, "text/html", "<meta charset='UTF-8'>SSID: " + wifi_ssid + "<br />device type:" + String(getDeviceFlagLabel(deviceFlag)) + "<br />已取得WiFi信息,正在尝试连接,请关闭此页面。"); //返回保存成功页面
...
bool scanWiFi() {
  ...
  if (n <= 0) {
    ...
    serial_writelog("%d networks found\r\n", n);
    for (int i = 0; i < n; ++i) {
